...
Tab.Join_Events:addDropdown({
    title = "Select Events",
    list = {"Christmas Event","Assassin Event"}, 
...
Tab.Join_Events:addMulitDropdown({
    title = "Select Map",
    list = EventDropdown, 
        ...
        local e, s = pcall(function()
            if workspace:FindFirstChild("_UNITS") then
                ...
                if GetLevelData._is_map_or_portal_level == true then
                    ...
                    MacroMapSelect = tostring(SaveSettings['Macro']['Macro Map'][GameType][NameMap]) or tostring(PortalDataID[SaveSettings['Join']['Select Portals']])
                elseif GetLevelData._event_contract ~= nil then
                    NameMap = GetLevelData.map
                    if NameMap == "assassin_park" then
                        NameMap = GetLevelData.map
                        if NameMap == "assassin_park" then NameMap = "Assassin Park" end
                    else
...
if workspace._MAP_CONFIG.IsLobby.Value then
    ...
    task.spawn(function()
        while task.wait() do
            ...
            --Events
            if SaveSettings['Join']['Auto Join'] and SaveSettings['Join']['Enable Farm Events'] and SaveSettings["Join"]['Select Events'] == "Assassin Event" then
                task.wait(2)
                repeat wait()
                    local Data_Contract = game:GetService("ReplicatedStorage").endpoints.client_to_server.get_current_event_contracts:InvokeServer()
                    for i,v in pairs(game:GetService("ReplicatedStorage").endpoints.client_to_server.get_current_event_contracts:InvokeServer()) do
            ...
            if SaveSettings['Join']['Auto Join'] and SaveSettings['Join']['Enable Farm Events'] and SaveSettings["Join"]['Select Events'] == "Christmas Event" then
                    ...
                    repeat task.wait() 
                        ...
                        if SaveSettings['Join']['Auto Retry In Games'] then
                            ...
                            elseif GetLevelData._event_contract ~= nil then
                                local Data_Contract = game:GetService("ReplicatedStorage").endpoints.client_to_server.get_current_event_contracts:InvokeServer()
                                for i,v in pairs(game:GetService("ReplicatedStorage").endpoints.client_to_server.get_current_event_contracts:InvokeServer()) do
                                    if v["_portal_depth"] == number then
                                        local args = {
                                            [1] = tostring(i)
                                        }
                                        game:GetService("ReplicatedStorage").endpoints.client_to_server.open_event_contract_portal:InvokeServer(unpack(args))
