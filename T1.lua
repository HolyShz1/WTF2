...
local SaveSettings = {
    ["Join"] = {
        ...
        ['Select Challenges MAP'] = {},
        ['Select Act Challenges'] = {},
        ['Select Challenges'] = {},
...
local UI = Venyx.new({
    title = "[CHRISTMAS + TOURNAMENT] Anime Adventures",
...
------------------------------------------------------ [[ Main Gui ]] ------------------------------------------------------
local StoryMacroMap,MapData,StoryDropdown = {},{},{}
...
--Portals
local PortalDropdown,PortalID,PortalMacromap = {},{},{"7ds_portal",}
for v2, v3 in pairs(game:GetService("ReplicatedStorage").src.Data.Items:GetDescendants()) do
	if v3:IsA("ModuleScript") then
		for v4, v5 in pairs(require(v3)) do
            if v5['item_group'] == "Portals" then
                ...
                PortalID[PortalDropdown[v2]] = v5['id']
            end
		end
	end
end
--Challenge
local ChallengeDropdown,ChallengeDropdownID,ChallengeID = {},{},{}
...
table.foreach(ChallengeData['challenges'],function(i,v)
    ChallengeDropdownID = v['name']
    ChallengeID[ChallengeDropdownID] = i
	table.insert(ChallengeDropdown,v['name'])
end)
...
Tab.Join_Challenges:addMulitDropdown({
    title = "Select Challenges MAP",
    list = StoryDropdown, 
    default = SaveSettings['Join']['Select Challenges MAP'] ,
    callback = function(v)
        SaveSettings['Join']['Select Challenges MAP']  = v
        Save()
    end;
})
Tab.Join_Challenges:addMulitDropdown({
    title = "Select Challenges Act",
    list = {"Act 1","Act 2","Act 3","Act 4","Act 5","Act 6"},  
    default = SaveSettings['Join']['Select Act Challenges'] ,
    callback = function(v)
        SaveSettings['Join']['Select Act Challenges']  = v
        Save()
    end;
})
Tab.Join_Challenges:addMulitDropdown({
...
Tab.Auto_Claim:addToggle({
    ...
    default  = SaveSettings['Summon']['Auto Claim Quests'] ,
    callback = function(v)
        SaveSettings['Summon']['Auto Claim Quests'] = v
        Save()
    end ,
})
Tab.Auto_Claim:addToggle({
    title = "Auto Claim Unit Index",
    default  = SaveSettings['Summon']['Auto Claim Unit Index'] ,
    callback = function(v)
        SaveSettings['Summon']['Auto Claim Unit Index'] = v
        Save()
    end ,
})
Tab.Auto_Claim:addToggle({
    title = "Auto Claim Level Rewards",
    default  = SaveSettings['Summon']['Auto Claim Level Rewards'] ,
    callback = function(v)
        SaveSettings['Summon']['Auto Claim Level Rewards'] = v
        Save()
    end ,
})
------------------------------------------------------ [[ Summon ]] ------------------------------------------------------
                ...
                if SaveSettings['Macro']['Play Macro'] and isfile("HolyShz/AnimeAdventures/Macro/" .. tostring(MacroMapSelect)..".json") then
                    ...
                    for i,v in pairs(Macro) do
                        ...
                        elseif v["Method_Data"]["Method"] == "Priority" then
                            local MacroData = v["Method_Data"]["Data"]
                            for index, value in pairs(workspace._UNITS:GetChildren()) do
                                if value:FindFirstChild("NumberTower") and value.NumberTower.Value == tonumber(MacroData["UnitPlace"]) then
                                    local _stats = value:FindFirstChild("_stats")
                                    repeat task.wait()
                                    ...
                                    until _stats:FindFirstChild("priority").Value == MacroData["Priority"]
                                    break
                                end
                            end
        ...
        while task.wait() do
            ...
            if SaveSettings['Join']['Auto Join'] and SaveSettings['Join']['Enable Farm Challenges'] then
                for i,v in pairs(game:GetService("Workspace"):FindFirstChild("_CHALLENGES"):FindFirstChild("Challenges"):GetChildren()) do
                    local Room = v
                    local MapNow = v:FindFirstChild('Level')
                    local ChallengeNow = v:FindFirstChild('Challenge')
                    local ChallengeMapID = MapData[SaveSettings['Join']['Select Challenges Map']].."_"..(SaveSettings['Join']['Select Act Challenges']:lower():gsub("act ","level_"))
                    local ChallengeID = ChallengeID[SaveSettings['Join']['Select Challenges']]
                    ...
                    if table.find(ChallengeMapID,MapNow.Value) and table.find(ChallengeID,ChallengeNow.Value) then
                    ...
                    repeat task.wait()
                        if SaveSettings['Join']['Auto Abilite at Wave'] and tonumber(SaveSettings['Join']['Select Wave Use Abilite']) <= _wave.Value then
                            for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
                                ...
                                elseif v._stats.id.Value == "pucci_heaven" or v._stats.id.Value == "dio_heaven" or v._stats.id.Value == "eren_final" then
                                    if tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name then
