...
if table.find(PlaceId_AD,game.PlaceId) then
    ...
    function CheckInMap()
        print('Check in Map : Update...')
        plr.PlayerGui:WaitForChild("PAGES")
        return plr.PlayerGui.PAGES.MatchResultPage.Visible == false and (Values.GAME_WAVE.value == "Starting" or Values.GAME_WAVE.value.min == 0)
    end
    
    --ItemWebhook--
    local PlayerDataGem = game:GetService("ReplicatedStorage").Remotes.GetInventory:InvokeServer()
    local LevelOld = PlayerDataGem.Level
    local GemOld = PlayerDataGem.Currencies.Gems
    local TraitOld = PlayerDataGem['Items']['Trait Crystal']
    local RiskyOld = PlayerDataGem['Items']['Risky Dice']
    local FrostOld = PlayerDataGem['Items']['Frost Bind']
    ...
    Main_Misc_Page:addToggle({
        title = "Auto Vote Start/Skip Wave",
        default = SaveSettings["Main"]['Auto Start_Skip'] ,
        callback = function(v)
            SaveSettings["Main"]['Auto Start_Skip'] = v
            Save()
        end;
    })
    Main_Misc_Page:addToggle({
        title = "Auto Next Level",
        default = SaveSettings["Main"]['Auto Next Level'],
        callback = function(v)
            SaveSettings["Main"]['Auto Next Level'] = v
            Save()
        end;
    })
    Main_Misc_Page:addToggle({
    ...
    function WebhookLeave()
        if SaveSettings["Notifier"]["Send After End Game"] then
            ...
            LeaveWaveEnd = tostring(game:GetService("Players").LocalPlayer.PlayerGui.WaveTopBar.Wave.WaveFrame.TextLabel.Text)
            LeaveModeName = tostring(game:GetService("Players").LocalPlayer.PlayerGui.HUD.HolderLeftBottom.ServerInformationFrame.GameModeLabel.text)
            LeaveMapName = tostring(game:GetService("Players").LocalPlayer.PlayerGui.MatchIntroGui.Holder.Map.TextLabel.Text)
            LeaveTimeEnd = tostring(FormatTime(workspace.DistributedGameTime))
            ...
            if (PlayerDataWH2.Level - LevelOld) > 0 then
                TextDropLabel = TextDropLabel .. tostring(CountAmount) .. ". Level + " .. comma_value(PlayerDataWH2.Level - LevelOld) .. " :sparkles:\n"
                CountAmount = CountAmount + 1
            end
            if (PlayerDataWH2.Currencies.Gems - GemOld) > 0 then
                TextDropLabel = TextDropLabel .. tostring(CountAmount) .. ". GEM + " .. comma_value(PlayerDataWH2.Currencies.Gems - GemOld) .. " [Total : " .. comma_value(PlayerDataWH2.Currencies.Gems) .."] <:Gem:1245532591981858916>\n"
                CountAmount = CountAmount + 1
            end
            if (PlayerDataWH2['Items']['Trait Crystal'] - TraitOld) > 0 then
                TextDropLabel = TextDropLabel .. tostring(CountAmount) .. ". Trait Crystal + " .. comma_value(PlayerDataWH2['Items']['Trait Crystal'] - TraitOld) .. " [Total : " .. comma_value(PlayerDataWH2['Items']['Trait Crystal'] or 0) .."] <:TraitCrystal:1245532370116018236>\n"
                CountAmount = CountAmount + 1
            end
            ...
            if TextDropLabel == "" then
                ...
                local DataLeaveMap = {
                    ...
                    ["embeds"] = {
                        {
                            ["title"] = "**Anime Defenders : Wave " .. tostring(LeaveWaveEnd) .. "**",
                            ["author"] = {
                                ["name"] = "HolyShz Webhook - Anime Defenders ",
                                ["icon_url"] = "https://tr.rbxcdn.com/2aa3ca9d432fc51be06467d8eecb5c3d/150/150/Image/png",
                            },
                            ["thumbnail"] = {
                            ...
                            ["fields"] = {
                                ...
                                },
                                {
                                    ...
                                    ["value"] = "\n• :hourglass:" .. " Time Elapsed -> " .. tostring(LeaveTimeEnd) .. " - Wave " .. tostring(LeaveWaveEnd) ..
                                                "\n• :earth_americas:" .. " Map -> " .. tostring(LeaveMapName) .. " - " .. tostring(LeaveModeName)
                                        ,
                                    ["inline"] = false
                                },
                                {
                ...
                local DataLeaveMap = game:GetService('HttpService'):JSONEncode(DataLeaveMap)
                local HttpRequest = http_request or request or HttpPost or syn.request;
                HttpRequest({Url=SaveSettings["Notifier"]["Webhook Url"], Body=DataLeaveMap, Method="POST", Headers=Headers})
                print('🟢🟢🟢 Webhook Leave Sending Resulte ... !!! 🟢🟢🟢')
                local Players = game:GetService("Players")
                ...
                print("Leave at Wave " .. tostring(game:GetService("Players").LocalPlayer.PlayerGui.WaveTopBar.Wave.WaveFrame.TextLabel.Text))
            end
        end

    function LeaveAtWave()
        local Players = game:GetService("Players")
        local player = Players.LocalPlayer
        local TeleportService = game:GetService("TeleportService")
        wait(tonumber(tostring(SaveSettings["Main"]["Delay [Start,Retry,Leave]"])))
    ...
    function WebhookLobby()
        ...
        local TestDataLobby = {
            ...
            ["embeds"] = {
                {
                    ["title"] = "**Anime Defenders : TEST Webhook**",
                    ["author"] = {
                        ["name"] = "HolyShz Webhook - Anime Defenders ",
                        ["icon_url"] = "https://tr.rbxcdn.com/2aa3ca9d432fc51be06467d8eecb5c3d/150/150/Image/png",
                    },
    ...
    WebhookPage:addButton({
        ...
        callback = function(v)
            if game.PlaceId ~= 17017769292 then
                print("send Webhook Leave")
                pcall(function() 
                    WebhookLobby()
                end)
            else
                print("send Webhook Lobby")
                pcall(function() 
                    WebhookLobby()
                end)
            end
    ...
    spawn(function()
        while wait() do
            ...
            if LocalPlayer.PlayerGui:FindFirstChild('PAGES'):FindFirstChild('MatchResultPage') and LocalPlayer.PlayerGui.PAGES.MatchResultPage.Visible and SaveSettings["Notifier"]["Send After End Game"] then
                ...
                local PlayerItemsDataAll = game:GetService("ReplicatedStorage").Remotes.GetInventory:InvokeServer()
                ...
                for i,v in pairs(game:GetService("Players").LocalPlayer.PlayerGui.PAGES.MatchResultPage.Main.Rewards:GetChildren()) do
                    if v:IsA("Frame") then
                        ...
                        --addnewitem
                        if v.Name == "Gems" then
                            TextDropLabel = TextDropLabel .. tostring(CountAmount) .. ". " .. v.Button.NameLabel.Text .. " + " .. comma_value(PlayerItemsDataAll.Currencies.Gems - GemOld) .. " [Total : " .. comma_value(PlayerItemsDataAll.Currencies.Gems) .."] <:Gem:1245532591981858916>\n"
                            CountAmount = CountAmount + 1
                        elseif v.Name == "Trait Crystal" then
                            TextDropLabel = TextDropLabel .. tostring(CountAmount) .. ". " .. v.Button.NameLabel.Text .. " + " .. comma_value(PlayerItemsDataAll['Items']['Trait Crystal'] - TraitOld) .. " [Total : " .. PlayerItemsDataAll['Items'][tostring(v.Name)] .."] <:TraitCrystal:1245532370116018236>\n"
                            CountAmount = CountAmount + 1
                        elseif v.Name == "Risky Dice" then
                            TextDropLabel = TextDropLabel .. tostring(CountAmount) .. ". " .. v.Button.NameLabel.Text .. " + " .. comma_value(PlayerItemsDataAll['Items']['Risky Dice'] - RiskyOld) .. " [Total : " .. PlayerItemsDataAll['Items'][tostring(v.Name)] .."] <:Dice:1245532589226197042>\n"
                            CountAmount = CountAmount + 1
                        elseif v.Name == "Frost Bind" then
                            TextDropLabel = TextDropLabel .. tostring(CountAmount) .. ". " .. v.Button.NameLabel.Text .. " + " .. comma_value(PlayerItemsDataAll['Items']['Frost Bind'] - FrostOld) .. " [Total : " .. PlayerItemsDataAll['Items'][tostring(v.Name)] .."] <:FrostBind:1245534927487897640>\n"
                            CountAmount = CountAmount + 1
                    ...
                    if (PlayerItemsDataAll.Level - LevelOld) > 0 then
                        TextDropLabel = TextDropLabel .. tostring(CountAmount) .. ". Level + " .. comma_value(PlayerItemsDataAll.Level - LevelOld) .. " :sparkles:\n"
                        CountAmount = CountAmount + 1
                    end
                end
                            ...
                            for i2,v2 in pairs(PlayerDataKillsInfo['Units']) do
                                if tostring(v1) == tostring(i2) then
                                    ...
                                    if v2.Worthiness == nil then
                                        v2.Worthiness = 0
                                        TextUnitKills = TextUnitKills .. "[Lv." .. tostring(v2.Level) .. "] • (" .. (LevelXPNow) .. "/" .. tostring(LevelXPNext) ..") •  __" .. tostring(v2.Type) .. "__ | " .. Traits1_Resulte .. "" .. Traits2_Resulte .. " | [" .. DMGRanks .. "] [" .. CDRanks .. "] ["  .. RNGRanks .. "] | [Worthiness : " .. tostring(string.format('%.2f',v2.Worthiness)) .. "%]\n"
                                    else
                                        TextUnitKills = TextUnitKills .. "[Lv." .. tostring(v2.Level) .. "] • (" .. (LevelXPNow) .. "/" .. tostring(LevelXPNext) ..") •  __" .. tostring(v2.Type) .. "__ | " .. Traits1_Resulte .. "" .. Traits2_Resulte .. " | [" .. DMGRanks .. "] [" .. CDRanks .. "] ["  .. RNGRanks .. "] | [Worthiness : " .. tostring(string.format('%.2f',v2.Worthiness)) .. "%]\n"
                            ...
                            ["fields"] = {
                                ...
                                },
                                {
                                    ...
                                    ["value"] = "\n• Mode -> " .. tostring(game:GetService("Players").LocalPlayer.PlayerGui.HUD.HolderLeftBottom.ServerInformationFrame.GameModeLabel.text) ..
                                                ...
                                                "\n• Challenge -> " .. tostring(game:GetService("Players").LocalPlayer.PlayerGui.PAGES.MatchResultPage.Main.ChallangeLabel.Text or 'nil') ..
                ...
                local PlayerData_Setting = game:GetService('HttpService'):JSONEncode(PlayerData_Setting)
                local HttpRequest = http_request or request or HttpPost or syn.request;
                HttpRequest({Url=SaveSettings["Notifier"]["Webhook Url"], Body=PlayerData_Setting, Method="POST", Headers=Headers})
                wait(3000)
            end
        end
    end)
    ...
    function ClickNext()
        QuestBoard = game:GetService("Players").LocalPlayer.PlayerGui.PAGES.MatchResultPage.Main.Options.PlayNextButton
        QuestBoard.Position = UDim2.new(0, -700 ,0, -700)
        QuestBoard.Size = UDim2.new(100,100,100,100)
        QuestBoard.BackgroundTransparency = 1
        wait();
    ...
    function ClickEnterRaid()
        QuestBoard = game:GetService("Players").LocalPlayer.PlayerGui.PromptGui.PromptDefault.Holder.Options.Enter
        QuestBoard.Position = UDim2.new(0, -700 ,0, -700)
        QuestBoard.Size = UDim2.new(100,100,100,100)
        QuestBoard.BackgroundTransparency = 1
        wait();
    ...
    spawn(function()
        while task.wait() do
            ...
            local MainTimeChallenge = 3600 - (0 + ostime.min * 60 + ostime.sec);
            if MainTimeChallenge > 3000 then
                local Time_RAID_ENDS_TIME = MainTimeChallenge - 3000;
                if Time_RAID_ENDS_TIME <= 0 then 
                    CheckRaidsOpen = false
                    Text_Raids_Boss.Options:ChangeText('Challenge : Open IN : N/S')
                else
                    CheckRaidsOpen = true
                    Text_Raids_Boss.Options:ChangeText('Challenge : Open IN : '.. convertToHMS(MainTimeChallenge, false, false, true))
                end
            else
                if MainTimeChallenge <= 0 then
                    Text_Raids_Boss.Options:ChangeText('Challenge : Expires IN : N/S')
                else
                    Text_Raids_Boss.Options:ChangeText('Challenge : Expires IN : '.. convertToHMS(MainTimeChallenge, false, false, true))
                end;	
                CheckRaidsOpen = false	
            end
                ...
                if SubStuff_DONT_DELETE then 
                    for i,v in pairs(LocalPlayer.PlayerGui:FindFirstChild("UI"):FindFirstChild("GUIs"):GetChildren()) do
                        ...
                        if v.Adornee:GetAttribute("RaidId") and TextHolder:FindFirstChild('RaidLockedFrame') and TextHolder:FindFirstChild('RaidLockedFrame'):FindFirstChild('UnlockLabel').Visible and MatchDisplayFrame then
                            if MatchDisplayFrame.MatchDetailsFrame.MapNameLabel.Text == SaveSettings["Main"]['Select Raids'] then
                                if Values.CURRENT_MATCH.value ~= nil then
                                    if SaveSettings["Main"]['Auto Start'] then
                                        wait(tonumber(tostring(SaveSettings["Main"]["Delay [Start,Retry,Leave]"])))
                                        FireEvent("START_MATCH")
            ...
            if game.PlaceId ~= 17017769292 and game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("PAGES") then  
                ...
                if SaveSettings["Main"]['Auto Leave at Wave'] then
                    ...
                    MapValuesEX2 = require(game:GetService("ReplicatedStorage").Values)
                    if MapValuesEX2['GAME_WAVE']['value']['min'] >= tonumber(SaveSettings["Main"]['Select Wave Amount']) then
                        pcall(function() 
                            WebhookLeave()
                            LeaveAtWave()
                        end)
                ...
                --Start / Skip Vote
                if IsMatch() and SaveSettings["Main"]['Auto Start_Skip'] then
                    FireEvent("GAME_SKIPROUND")
                end

                --Auto Next
                if SaveSettings["Main"]['Auto Next Level'] and game:GetService("Players").LocalPlayer.PlayerGui.PAGES.MatchResultPage.Visible then
                    wait(3)
                    wait(tonumber(tostring(SaveSettings["Main"]["Delay [Start,Retry,Leave]"])))
                    print('Play Next Match ..!!!')
                    FireEvent("PLAY_NEXT_GAME")
                    wait(1)
                    ClickNext()
                    wait()
                    Replay()
                ...
                if SaveSettings["Main"]['Auto Leave'] and game:GetService("Players").LocalPlayer.PlayerGui.PAGES.MatchResultPage.Visible then
                    wait(3)
                    wait(tonumber(tostring(SaveSettings["Main"]["Delay [Start,Retry,Leave]"])))
                    FireEvent("GAME_MODE_SELECTED_CTS", "NormalLobby")
                    wait(tonumber(tostring(SaveSettings["Main"]["Delay [Start,Retry,Leave]"])))
                    wait(1)
                    ClickLeave()
                    wait(30)
                ...
                --Clicknormal
                if SaveSettings["Main"]['Manual Click [Next]'] and game:GetService("Players").LocalPlayer.PlayerGui.PAGES.MatchResultPage.Visible == true then
                    wait(3)
                    wait(tonumber(tostring(SaveSettings["Main"]["Delay [Start,Retry,Leave]"])))
                    print('Play Next Match ..!!!')
                    FireEvent("PLAY_NEXT_GAME")
                    ...
                    wait(tonumber(tostring(SaveSettings["Main"]["Delay [Start,Retry,Leave]"])))
                    wait()
                    ClickPlayNext =  LocalPlayer.PlayerGui.PAGES.MatchResultPage.Main.Options.PlayNextButton
                    repeat
                        wait()
                        game:GetService("VirtualInputManager"):SendMouseButtonEvent(
                            ClickPlayNext.AbsolutePosition.X + 50, 
                            ClickPlayNext.AbsolutePosition.Y + 50, 
                            0, 
