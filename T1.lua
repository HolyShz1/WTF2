...
local SaveSettings = {
    ["Join"] = {
        ...
        ['Select World Legends'] = "",
        ['Select Act Legends'] = "Act 1",
        ['Enable Farm Legend'] = false,
        ...
        ['Select Difficulty Castle'] = "Normal",
        ['Enable Farm InfinityCastle'] = false,
...
--Start_Assasin_Event
local function StartAssasin()
    for i,v in pairs(GetAssasinData()) do
        if v['map'] == "assassin_park" then v['_location_name'] = "Assassin Park" end
...
--Replay_Assasin_Event
local function ReplayAssasin()
    for i,v in pairs(GetAssasinData()) do
        if v['map'] == "assassin_park" then v['_location_name'] = "Assassin Park" end
        if table.find(SaveSettings['Join']['Select Event Challenges'],v['_challengename']) and table.find(SaveSettings['Join']['Select Event Map'],v['_location_name']) and table.find(SaveSettings['Join']['Select Event Tier'],tostring(v['_portal_depth'])) then
            local args = {
        ...
        for i, v in pairs(game:GetService("Players")[game.Players.LocalPlayer.Name].PlayerGui.InfiniteTowerUI.LevelSelect.InfoFrame.LevelButtons:GetChildren()) do
            if v.Name == "FloorButton" then
                if v.clear.Visible == false and v.Locked.Visible == false then
                    ...
                    local args = {
                        [1] = tonumber(room[2]),
                        [2] = SaveSettings['Join']['Select Difficulty Castle']
                    }
...
function WebhookMain()
    if SaveSettings["Webhook"]["Send After End Game"] then
        if SaveSettings["Webhook"]["Webhook Url"] ~= "" then 
            ...
            if result == "VICTORY" then Color = 65280 end
            if result == "DEFEAT" then Color = 16711680 end
            ...
            local Mapname_Assasin = GetLevelData._location_name
            local MapID = GetLevelData.id
            
            if GetLevelData._gamemode == "infinite_tower" then
                Gamemode = GetLevelData.name
                MapID = GetLevelData.map
            end
            if GetLevelData._event_contract ~= nil then
                Mapname_Assasin = GetLevelData._location_name .. " - Tier " .. GetLevelData._portal_depth
            end
            ...
            local PlayerData = {
                ...
                ["avatar_url"] = "https://tr.rbxcdn.com/180DAY-9db40f9ba7e7ab010267ba6a37f76a79/256/256/Image/png/noFilter",
                ["embeds"] = {
                    {
                        ["author"] = {
                        ...
                        ["thumbnail"] = {
                            ["url"] = "https://tr.rbxcdn.com/180DAY-9db40f9ba7e7ab010267ba6a37f76a79/256/256/Image/png/noFilter",
                        },
...
local UI = Venyx.new({
    title = "[UPD 20 + DUNGEON] Anime Adventures",
...
local Tab = {
    ...
    Join_LegendStage = Page.Join:addSection({ title = "Join - Auto Legend Stage" }),
    Join_Challenges = Page.Join:addSection({ title = "Join - Auto Challenges" }),
    Join_Raids = Page.Join:addSection({ title = "Join - Auto Raids" }),
    Join_InfinityCastle = Page.Join:addSection({ title = "Join - Auto Infinity Castle" }),
    Join_Dungeons = Page.Join:addSection({ title = "Join - Auto Dungeons" }),
...
--All_Portal_Worlds
local WorldsAll = require(game:GetService("ReplicatedStorage").src.Data.Maps)
for i,v in pairs(WorldsAll) do
    PortalID[v['id']] = v['name']
end
--Christmas
local WorldsChristmas = require(game:GetService("ReplicatedStorage").src.Data.Maps.Maps_Christmas)
for i,v in pairs(WorldsChristmas) do
...
--Join_LegendStage
Tab.Join_LegendStage:addDropdown({
    title = "Select World",
    list = StoryDropdown, 
    default = SaveSettings['Join']['Select World Legends'],
    callback = function(v)
        SaveSettings['Join']['Select World Legends'] = v
        Save()
    end;
})
Tab.Join_LegendStage:addDropdown({
    ...
    default = SaveSettings['Join']['Select Act Legends'],
    callback = function(v)
        SaveSettings['Join']['Select Act Legends'] = v
        Save()
    end;
})
Tab.Join_LegendStage:addToggle({
    title = "Enable Farm Legend Stage",
    default  = SaveSettings['Join']['Enable Farm Legend'] ,
    callback = function(v)
        SaveSettings['Join']['Enable Farm Legend'] = v
        Save()
    end ,
})
...
--Join_Dungeons
Tab.Join_Dungeons:addDropdown({
    title = "Select Dungeons",
    list = {"Cursed Womb"}, 
    default = SaveSettings['Join']['Select Dungeons'],
    callback = function(v)
        SaveSettings['Join']['Select Dungeons'] = v
        Save()
    end;
})
Tab.Join_Dungeons:addToggle({
    title = "Enable Farm Dungeons",
    default  = SaveSettings['Join']['Enable Farm Dungeons'] ,
    callback = function(v)
        SaveSettings['Join']['Enable Farm Dungeons'] = v
        Save()
    end ,
})
...
--Join_InfinityCastle
Tab.Join_InfinityCastle:addDropdown({
    title = "Select Difficulty",
    list = {"Normal","Hard"}, 
    default = SaveSettings['Join']['Select Difficulty Castle'],
    callback = function(v)
        SaveSettings['Join']['Select Difficulty Castle'] = v
        Save()
    end;
})
Tab.Join_InfinityCastle:addToggle({
    title = "Enable Farm Infinity Castle",
    default  = SaveSettings['Join']['Enable Farm InfinityCastle'] ,
    callback = function(v)
        SaveSettings['Join']['Enable Farm InfinityCastle'] = v
        Save()
    end ,
})
        ...
        local e, s = pcall(function()
            if workspace:FindFirstChild("_UNITS") then
                ...
                if GetLevelData._is_map_or_portal_level == true then
                    NameMap =  tostring(PortalID[GetLevelData.map]) or GetLevelData._location_name
                ...
                if MacroMapSelect == nil then MacroMapSelect = SaveSettings['Macro']['Macro Select'] end
                ...
                Text_MacroMap.Options:ChangeText("MODE : Macro - " .. GameType .." \nMAP : " .. NameMap)
                ...
                if SaveSettings['Macro']['Play Macro'] and isfile("HolyShz/AnimeAdventures/Macro/" .. tostring(MacroMapSelect)..".json") then
                    ...
                    for i,v in pairs(Macro) do
                        ...
                        if v["Method_Data"]["Method"] == "Place" then
                            ...
                            if string.find(v["Method_Data"]["Data"]["UnitPlace"],"hoshino") then
                                local args = {
                                    [1] = Table_Equipped_Slot[MacroData["UnitPlace"]]['Unit_uuid'],
                                    [2] = StringToCFrame(MacroData["CFrame"]) * CFrame.new(0, -1.15, 0) * CFrame.Angles(0, -0, -0)
                                }
                                game:GetService("ReplicatedStorage").endpoints.client_to_server.spawn_unit:InvokeServer(unpack(args))
                                task.wait(0.1)
                            elseif string.find(MacroData["UnitPlace"],"metal_knight_evolved") then
                                local args = {
                                    [1] = Table_Equipped_Slot[MacroData["UnitPlace"]]['Unit_uuid'],
                                    [2] = StringToCFrame(MacroData["CFrame"]) * CFrame.new(0, 0, 0) * CFrame.Angles(0, -0, -0)
                                }
                                game:GetService("ReplicatedStorage").endpoints.client_to_server.spawn_unit:InvokeServer(unpack(args))
                                task.wait(1.5)
                            elseif string.find(MacroData["UnitPlace"],"elyssia_evolved") then
                                local args = {
                                    [1] = Table_Equipped_Slot[MacroData["UnitPlace"]]['Unit_uuid'],
                                    [2] = StringToCFrame(MacroData["CFrame"]) * CFrame.new(0, -0.30, 0) * CFrame.Angles(0, -0, -0)
                                }
                                game:GetService("ReplicatedStorage").endpoints.client_to_server.spawn_unit:InvokeServer(unpack(args))
                                task.wait(1.5)
                            else
                        ...
                        elseif v["Method_Data"]["Method"] == "Priority" then
                            ...
                            for index, value in pairs(workspace._UNITS:GetChildren()) do
                                if value:FindFirstChild("NumberTower") and value.NumberTower.Value == tonumber(v["Method_Data"]["Data"]["UnitPlace"]) then
                                    ...
                                    repeat task.wait()
                                        MacroStats.Options:ChangeText('Status: Playing Marcro [ ' .. v["Method_Data"]["Method"] .. " ] [" .. v["Index"] .. "/" .. math.max(unpack(TableIndex)) .."]\nTower : " .. value.Name .. "\n Waiting For : $" .. tonumber( Module_Units[_stats:WaitForChild("id").Value]['upgrade'][tonumber(MacroData['Upgrade'])]["cost"] ) .. " , Time : " ..string.format("%.2f", Timelapse() - StartRecordTime) .. "/" .. string.format("%.2f", tonumber(v["Time"] + tonumber(SaveSettings["Macro"]['Delat Macro Play Time (sec)']))) )
                                    until workspace:FindFirstChild("_wave_num").Value >= tonumber(MacroData["Wave"])
...
local GAME_MODE = {'story','infinite','infinite_tower','raid'}
...
for i,GAME_MODE in pairs(GAME_MODE) do
    ...
    elseif GAME_MODE == "infinite_tower" then
...
if workspace._MAP_CONFIG.IsLobby.Value then
        ...
        while task.wait() do
            if getLobby() and SaveSettings['Join']['Auto Join'] and SaveSettings['Join']['Enable Farm Portals'] then
                ...
                for i,v in pairs(PortalsData) do
                    if string.find(v['item_id'],"portal") or string.find(v['item_id'],"disc") then
                        if v['item_id'] == PortalDataID[SaveSettings['Join']['Select Portals']] then
                            if v["_unique_item_data"]["_unique_portal_data"]["challenge"] == nil then

                                local string_1 = GetPortals(PortalDataID[SaveSettings['Join']['Select Portals']])[1]["uuid"];
                                    local table_1 = {
                                        ["friends_only"] = SaveSettings['Join']['Friends Only'] 
                                    };
    ...
    task.spawn(function()
        while task.wait() do
            ...
            --Story
            if SaveSettings['Join']['Auto Join'] and SaveSettings['Join']['Enable Farm Story&Legend'] then
            ...
            --LegendStage
            if SaveSettings['Join']['Auto Join'] and SaveSettings['Join']['Enable Farm Legend'] then
                local Lobby = getLobby()
                task.wait(2)
                repeat wait()
                    local process = MapData[SaveSettings['Join']['Select World Legends']].."_"..(SaveSettings['Join']['Select Act Legends']:lower():gsub("act ","legend_"))
                    local Type = "Hard"
                     print(process)
                     print(Levels[process]["id"])
                    ...
                    wait(5)
                until SaveSettings['Join']['Auto Join'] or not SaveSettings['Join']['Auto Join'] or not SaveSettings['Join']['Enable Farm Legend']
                game:GetService("ReplicatedStorage").endpoints.client_to_server.request_leave_lobby:InvokeServer(Lobby.Name)
            end
            --Raids
            ...
            --Infinity Castle
            if SaveSettings['Join']['Auto Join'] and SaveSettings['Join']['Enable Farm InfinityCastle'] then
                task.wait(2)
                repeat wait()
                    ...
                    wait(5)
                until SaveSettings['Join']['Auto Join'] or not SaveSettings['Join']['Auto Join'] or not SaveSettings['Join']['Enable Farm InfinityCastle']
            end
            --Dungeons
            if SaveSettings['Join']['Auto Join'] and SaveSettings['Join']['Enable Farm Dungeons'] then
                --local Lobby = getDungeonLobby()
                local Lobby = SaveSettings["Join"]['Select Dungeons'] == 'Cursed Womb' and "_lobbytemplate_event222"
                task.wait(2)
                repeat wait()

                    local string_1 = SaveSettings["Join"]['Select Dungeons'] == 'Cursed Womb' and "_lobbytemplate_event222";
                    local table_1 = {
                        ["selected_key"] = SaveSettings["Join"]['Select Dungeons'] == 'Cursed Womb' and "key_jjk_finger"
                    };
                    local Target = game:GetService("ReplicatedStorage").endpoints["client_to_server"]["request_join_lobby"];
                    Target:InvokeServer(string_1, table_1);

                    wait(120)
                until SaveSettings['Join']['Auto Join'] or not SaveSettings['Join']['Auto Join'] or not SaveSettings['Join'] ['Enable Farm Dungeons']
                    ...
                    if SaveSettings['Join']['Auto Abilite at Wave'] and tonumber(SaveSettings['Join']['Select Wave Use Abilite']) <= _wave.Value then
                        for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
                            ...
                            if tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name then
                                if v._stats.id.Value ~= "erwin" and v._stats.id.Value ~= "wendy" and v._stats.id.Value ~= "leafa" and v._stats.id.Value ~= "leafa_evolved" and v._stats.id.Value ~= "aot_generic"
                                and v._stats.id.Value ~= "pucci_heaven" and v._stats.id.Value ~= "dio_heaven" and v._stats.id.Value ~= "eren_final" and v._stats.id.Value ~= "aizen_christmas" then
                                ...
                                elseif v._stats.id.Value == "pucci_heaven" or v._stats.id.Value == "dio_heaven" or v._stats.id.Value == "eren_final" or v._stats.id.Value == "aizen_christmas" then
                                    if tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name then
                                        if v._stats.state.Value ~= "attack" then
                                            -- Check Infinite
                                            if GetLevelData._gamemode == "infinite" then
                                            ...
                                            -- Check Raid
                                            elseif GetLevelData._gamemode == "raid" then
                                                if GetWaveNumber() % 15 == 0 or GetWaveNumber() % 30 == 0 then
                                                    wait(tonumber(SaveSettings["Join"]['Delay Use Abilite (sec)']))
                                                    game:GetService("ReplicatedStorage").endpoints.client_to_server.use_active_attack:InvokeServer(v)
                                                    warn("Use Skill : " ..tostring(v._stats.id.Value) .. " | Wave : " ..tostring(_wave.Value))
                                                elseif GetWaveNumber() % 20 == 0 or GetWaveNumber() % 30 == 0 then
                                                    wait(tonumber(SaveSettings["Join"]['Delay Use Abilite (sec)']))
                                                    game:GetService("ReplicatedStorage").endpoints.client_to_server.use_active_attack:InvokeServer(v)
                                                    warn("Use Skill : " ..tostring(v._stats.id.Value) .. " | Wave : " ..tostring(_wave.Value))
                                                elseif GetWaveNumber() == 15 or GetWaveNumber() == 30 then
                                                    wait(tonumber(SaveSettings["Join"]['Delay Use Abilite (sec)']))
                                                    game:GetService("ReplicatedStorage").endpoints.client_to_server.use_active_attack:InvokeServer(v)
                                                    warn("Use Skill : " ..tostring(v._stats.id.Value) .. " | Wave : " ..tostring(_wave.Value))
                                                elseif GetWaveNumber() == 20 or GetWaveNumber() == 30 then
                                                    wait(tonumber(SaveSettings["Join"]['Delay Use Abilite (sec)']))
                                                    game:GetService("ReplicatedStorage").endpoints.client_to_server.use_active_attack:InvokeServer(v)
                                                    warn("Use Skill : " ..tostring(v._stats.id.Value) .. " | Wave : " ..tostring(_wave.Value))
                                                end
                                            -- Check Story or Infinite Tower
                                            elseif GetLevelData._gamemode == "story" or "infinite_tower" then
                                                if game:GetService("Workspace")["_wave_num"].Value == 15 then
                                                    wait(tonumber(SaveSettings["Join"]['Delay Use Abilite (sec)']))
                                                    game:GetService("ReplicatedStorage").endpoints.client_to_server.use_active_attack:InvokeServer(v)
                                                    warn("Use Skill : " ..tostring(v._stats.id.Value) .. " | Wave : " ..tostring(_wave.Value))                                                                  
                                                end
            ...
            GameFinished:GetPropertyChangedSignal("Value"):Connect(function()
                if GameFinished.Value == true then
                    ...
                    repeat task.wait() 
                        ...
                        if SaveSettings['Join']['Auto Retry In Games'] then
                            if GetLevelData._event_contract ~= nil then
                                ...
                                if SaveSettings['Join']['Auto Retry In Games'] and SaveSettings['Join']['Auto Back To Lobby In Games'] then
                                    wait(10)
                                    TeleportLobby()
                                    wait(.5)
                            ...
                            if GetLevelData._is_map_or_portal_level == true then
                                local args = { [1] = "replay", [2] = { ["item_uuid"] = GetPortals(PortalDataID[SaveSettings['Join']['Select Portals']])[1]["uuid"] }}
                                game:GetService("ReplicatedStorage").endpoints.client_to_server.set_game_finished_vote:InvokeServer(unpack(args))
                                wait(.25)
                    ...
                    repeat task.wait()
                            ...
                            game:GetService("VirtualInputManager"):SendMouseButtonEvent(Center.X, Center.Y, 0, true, game.Workspace, 1)
                            game:GetService("VirtualInputManager"):SendMouseButtonEvent(Center.X, Center.Y, 0, false, game.Workspace, 1)
                            wait(.5)
                            game:GetService("VirtualInputManager"):SendMouseButtonEvent(Center.X, Center.Y, 0, true, game.Workspace, 0)
                            game:GetService("VirtualInputManager"):SendMouseButtonEvent(Center.X, Center.Y, 0, false, game.Workspace, 0)
                                    ...
                                    elseif v._stats.id.Value == "pucci_heaven" or v._stats.id.Value == "dio_heaven" or v._stats.id.Value == "eren_final" or v._stats.id.Value == "aizen_christmas" then
                                        if tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name then
                                            if v._stats.state.Value ~= "attack" then
                                                ...
                                                -- Check Raid
                                                elseif GetLevelData._gamemode == "raid" then
                                                    if GetWaveNumber() % 15 == 0 then
                                                        wait(tonumber(SaveSettings["Join"]['Delay Use Abilite (sec)']))
                                                        game:GetService("ReplicatedStorage").endpoints.client_to_server.use_active_attack:InvokeServer(v)
                                                        warn("Use Skill : " ..tostring(v._stats.id.Value) .. " | Wave : " ..tostring(_wave.Value))
                                                    elseif GetWaveNumber() % 20 == 0 then
                                                        wait(tonumber(SaveSettings["Join"]['Delay Use Abilite (sec)']))
                                                        game:GetService("ReplicatedStorage").endpoints.client_to_server.use_active_attack:InvokeServer(v)
                                                        warn("Use Skill : " ..tostring(v._stats.id.Value) .. " | Wave : " ..tostring(_wave.Value))
                                                    elseif GetWaveNumber() == 15 then
                                                        wait(tonumber(SaveSettings["Join"]['Delay Use Abilite (sec)']))
                                                        game:GetService("ReplicatedStorage").endpoints.client_to_server.use_active_attack:InvokeServer(v)
                                                        warn("Use Skill : " ..tostring(v._stats.id.Value) .. " | Wave : " ..tostring(_wave.Value))
                                                    elseif GetWaveNumber() == 20 then
                                                        wait(tonumber(SaveSettings["Join"]['Delay Use Abilite (sec)']))
                                                        game:GetService("ReplicatedStorage").endpoints.client_to_server.use_active_attack:InvokeServer(v)
                                                        warn("Use Skill : " ..tostring(v._stats.id.Value) .. " | Wave : " ..tostring(_wave.Value))
                                                    end
