                ...
                if SaveSettings['Macro']['Play Macro'] and isfile("HolyShz/AnimeAdventures/Macro/" .. tostring(MacroMapSelect)..".json") then
                    ...
                    for i,v in pairs(Macro) do
                        ...
                        if v["Method_Data"]["Method"] == "Place" then
                            ...
                            local MoneyPlace = (game:GetService("Players").LocalPlayer._stats.resource.Value >= tonumber(Module_Units[MacroData['UnitPlace']]['cost']) )
                            if GetLevelData._challenge == "double_cost" then
                                MoneyPlace = (game:GetService("Players").LocalPlayer._stats.resource.Value >= tonumber(Module_Units[MacroData['UnitPlace']]['cost'] * 1.5) )
                            else
                                MoneyPlace = (game:GetService("Players").LocalPlayer._stats.resource.Value >= tonumber(Module_Units[MacroData['UnitPlace']]['cost']) )
                            end
                        ...
                        elseif v["Method_Data"]["Method"] == "Upgrade" then
                            ...
                            for index, value in pairs(workspace._UNITS:GetChildren()) do
                                if value:FindFirstChild("NumberTower") and value.NumberTower.Value == tonumber(v["Method_Data"]["Data"]["UnitPlace"]) then
                                    ...
                                    local MoneyUpgrade = (game:GetService("Players").LocalPlayer._stats.resource.Value >= tonumber(Module_Units[_stats:WaitForChild("id").Value]['upgrade'][tonumber(MacroData['Upgrade'])]["cost"]) )
                                    if GetLevelData._challenge == "double_cost" then
                                        MoneyUpgrade = (game:GetService("Players").LocalPlayer._stats.resource.Value >= tonumber(Module_Units[_stats:WaitForChild("id").Value]['upgrade'][tonumber(MacroData['Upgrade'])]["cost"] * 1.5) )
                                    else
                                        MoneyUpgrade = (game:GetService("Players").LocalPlayer._stats.resource.Value >= tonumber(Module_Units[_stats:WaitForChild("id").Value]['upgrade'][tonumber(MacroData['Upgrade'])]["cost"]) )
                                    end
