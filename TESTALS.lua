...
if game.CoreGui:FindFirstChild("KeySystemGui") then game.CoreGui:FindFirstChild("KeySystemGui"):Destroy() end
    ...
    local SaveSettings = {
        ...
        ["Portals"] = {
            ...
            ['Select Map Portals SELL'] = {"Planet Nemak"},
    ...
    for i,v in pairs(TableData['ItemData']) do
        if v == nil or DataOld['ItemData']['PerfectStatCube']['Amount'] == nil or DataOld['ItemData']['StatCube']['Amount'] == nil 
        or DataOld['ItemData']['BerserkCapsule']['Amount'] == nil or DataOld['ItemData']['DemonCapsule']['Amount'] == nil 
        or DataOld['ItemData']['SummerCapsule']['Amount'] == nil or DataOld['ItemData']['SlimeCapsule']['Amount'] == nil then
            warn(i,v)
        else
            --WebhookOld
            --CurentlyOld
            ...
            local PerfectStatCubeOld = DataOld['ItemData']['PerfectStatCube']
            local StatCubeOld = DataOld['ItemData']['StatCube']
            ...
            local BerserkCapsuleOld = DataOld['ItemData']['BerserkCapsule']['Amount']
            local DemonCapsuleOld = DataOld['ItemData']['DemonCapsule']['Amount']
            local SummerCapsuleOld = DataOld['ItemData']['SummerCapsule']['Amount']
            local SlimeCapsuleOld = DataOld['ItemData']['SlimeCapsule']['Amount']
        end
    end
    ...
    Portals_Selling:addMulitDropdown({
        title = "Select Sell Map Portals",
        list = TableMap2, 
        default = SaveSettings["Portals"]['Select Map Portals SELL'],
        callback = function(v)
            SaveSettings["Portals"]['Select Map Portals SELL'] = v
            Save()
        end;
    })
    ...
    spawn(function()
        while wait() do
            if SaveSettings["Portals"]['Auto Sell Portals'] then
                PortalsDataSell = game:GetService("ReplicatedStorage").Remotes.GetPlayerData:InvokeServer()
                for i,v in pairs(PortalsDataSell['PortalData']) do
                    if table.find(SaveSettings["Portals"]['Select Tier Portals SELL'],v['PortalName']) and table.find(SaveSettings["Portals"]['Select Map Portals SELL'],v['PortalData']['Map']) and table.find(SaveSettings["Portals"]['Select Maps Act SELL'],tostring(v['PortalData']['Act'])) and table.find(SaveSettings["Portals"]['Select Challenge Portals SELL'],v['PortalData']['Challenges']) then
                        repeat wait() 
                            local args = {
                                [1] = v['PortalID']
                            }

                            game:GetService("ReplicatedStorage").Remotes.RemovePortal:FireServer(unpack(args))
                            ...
                            wait(.3)
                        until not SaveSettings["Portals"]['Auto Open Portals']
                    end
                end
            end
        end
