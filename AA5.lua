...
function WebhookMain()
    if SaveSettings["Webhook"]["Send After End Game"] then
        if SaveSettings["Webhook"]["Webhook Url"] ~= "" then 
            ...
            local WebhookGamemode = GetLevelData._gamemode

            if GetLevelData._event_contract ~= nil then
                WebhookGamemode = "EVENT_CONTRACT_Sakamoto - Tier " .. GetLevelData._portal_depth
            end
        ...
        local e, s = pcall(function()
            if workspace:FindFirstChild("_UNITS") then
                ...
                elseif GetLevelData._event_contract ~= nil then
                    ...
                    if GetLevelData.map == "assassin_park" then NameMap = "Assassin Park" end
                    GameType = GetLevelData._gamemode
                ...
                if SaveSettings['Macro']['Play Macro'] and isfile("HolyShz/AnimeAdventures/Macro/" .. tostring(MacroMapSelect)..".json") then
                    ...
                    for i,v in pairs(Macro) do
                        ...
                        if v["Method_Data"]["Method"] == "Place" then
                            ...
                            repeat task.wait()
                                MacroStats.Options:ChangeText('Status: Playing Marcro [ ' .. v["Method_Data"]["Method"] .. " ] [" .. v["Index"] .. "/" .. math.max(unpack(TableIndex)) .."]\nTower : " .. v["Method_Data"]["Data"]["UnitPlace"] .. "\n Waiting For : $" .. comma_value(tonumber(GetLevelData._challenge == "double_cost" and (Module_Units[MacroData['UnitPlace']]['cost'] * 1.5) or (Module_Units[MacroData['UnitPlace']]['cost']))) .. " , Time : " ..string.format("%.2f", Timelapse() - StartRecordTime) .. "/" .. string.format("%.2f", tonumber(v["Time"] + tonumber(SaveSettings["Macro"]['Delat Macro Play Time (sec)']))) )
                            until (game:GetService("Players").LocalPlayer._stats.resource.Value >= tonumber(GetLevelData._challenge == "double_cost" and (Module_Units[MacroData['UnitPlace']]['cost'] * 1.5) or (Module_Units[MacroData['UnitPlace']]['cost'])) )
                            local args = {
                        ...
                        elseif v["Method_Data"]["Method"] == "Upgrade" then
                            ...
                            for index, value in pairs(workspace._UNITS:GetChildren()) do
                                if value:FindFirstChild("NumberTower") and value.NumberTower.Value == tonumber(v["Method_Data"]["Data"]["UnitPlace"]) then
                                    ...
                                    repeat task.wait()
                                        MacroStats.Options:ChangeText('Status: Playing Marcro [ ' .. v["Method_Data"]["Method"] .. " ] [" .. v["Index"] .. "/" .. math.max(unpack(TableIndex)) .."]\nTower : " .. value.Name .. "\n Waiting For : $" .. tonumber( GetLevelData._challenge == "double_cost" and (Module_Units[_stats:WaitForChild("id").Value]['upgrade'][tonumber(MacroData['Upgrade'])]["cost"] * 1.5) or (Module_Units[_stats:WaitForChild("id").Value]['upgrade'][tonumber(MacroData['Upgrade'])]["cost"]) ) .. " , Time : " ..string.format("%.2f", Timelapse() - StartRecordTime) .. "/" .. string.format("%.2f", tonumber(v["Time"] + tonumber(SaveSettings["Macro"]['Delat Macro Play Time (sec)']))) )
                                    until (game:GetService("Players").LocalPlayer._stats.resource.Value >= tonumber( GetLevelData._challenge == "double_cost" and (Module_Units[_stats:WaitForChild("id").Value]['upgrade'][tonumber(MacroData['Upgrade'])]["cost"] * 1.5) or (Module_Units[_stats:WaitForChild("id").Value]['upgrade'][tonumber(MacroData['Upgrade'])]["cost"])) )
                                    local args = { [1] = value }
                    ...
                    repeat task.wait() 
                        ...
                        if SaveSettings['Join']['Auto Retry In Games'] then
                            ...
                            if GetLevelData._event_contract ~= nil then
                                ...
                                wait(.25)
                            elseif GetLevelData._is_map_or_portal_level == true then
