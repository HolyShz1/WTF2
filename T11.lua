...
local UI = Venyx.new({
    title = "[UPD 19.5] Anime Adventures",
    ...
    task.spawn(function()
        while task.wait() do
            ...
            if SaveSettings['Join']['Auto Join'] and SaveSettings['Join']['Enable Farm Events'] and SaveSettings["Join"]['Select Events'] == "Assassin Event" then
                task.wait(2)
                repeat wait()
                    DataContract = game:GetService("ReplicatedStorage").endpoints.client_to_server.get_current_event_contracts:InvokeServer()
                    ...
                    repeat task.wait() 
                        ...
                        if SaveSettings['Join']['Auto Retry In Games'] then
                            if GetLevelData._is_map_or_portal_level == true then
                                ...
                                wait(.25)
                            elseif GetLevelData._event_contract ~= nil then
                                DataContractReplay1 = game:GetService("ReplicatedStorage").endpoints.client_to_server.get_current_event_contracts:InvokeServer()
                                for i,v in pairs(DataContractReplay1) do
                                    if v['_event_contract']['contract_group'] == "Sakamoto" then
                                        if table.find(SaveSettings['Join']['Select Event Challenges'],v['_challengename']) and table.find(SaveSettings['Join']['Select Event Map'],v['_location_name']) and table.find(SaveSettings['Join']['Select Event Tier'],v['_portal_depth']) then
                                            local args = {
                                                [1] = i
