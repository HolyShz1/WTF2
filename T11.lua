                ...
                if SaveSettings['Macro']['Play Macro'] and isfile("HolyShz/AnimeAdventures/Macro/" .. tostring(MacroMapSelect)..".json") then
                    ...
                    for i,v in pairs(Macro) do
                        ...
                        if v["Method_Data"]["Method"] == "Place" then
                            ...
                            elseif v["Method_Data"]["Data"]["UnitPlace"] == "elyssia_evolved" then
                                local CFrame_X,CFrame_Y,CFrame_Z = StringToXYZ(MacroData["CFrame"]) 
                                warn("elyssia_evolved")
                                local args = {
                                    [1] = Table_Equipped_Slot[MacroData["UnitPlace"]]['Unit_uuid'],
                                    [2] = CFrame.new(Vector3.new(CFrame_X -1.5, CFrame_Y -0.3, CFrame_Z) )
                                }
                                game:GetService("ReplicatedStorage").endpoints.client_to_server.spawn_unit:InvokeServer(unpack(args))
                            elseif v["Method_Data"]["Data"]["UnitPlace"] == "metal_knight_evolved" then
                                local CFrame_X,CFrame_Y,CFrame_Z = StringToXYZ(MacroData["CFrame"]) 
                                warn("metal_knight_evolved")
                                local args = {
                                    [1] = Table_Equipped_Slot[MacroData["UnitPlace"]]['Unit_uuid'],
                                    [2] = CFrame.new(Vector3.new(CFrame_X - 1.5, CFrame_Y, CFrame_Z) )
                                }
                                game:GetService("ReplicatedStorage").endpoints.client_to_server.spawn_unit:InvokeServer(unpack(args))
                            else
                                local args = {
