...
local SaveSettings = {
    ["Join"] = {
        ...
        ['Auto Retry Assasin Event'] = false,
    ...
    if SaveSettings["Webhook"]["Send After End Game"] then
        if SaveSettings["Webhook"]["Webhook Url"] ~= "" then 
            ...
            if GetLevelData._event_contract ~= nil then
                WebhookGamemode = "EVENT_Sakamoto - Tier " .. GetLevelData._portal_depth
            end
...
Tab.Join_SettingInGames:addToggle({
    title = "Auto Retry [Assasin Event]",
    default  = SaveSettings['Join']['Auto Retry Assasin Event'] ,
    callback = function(v)
        SaveSettings['Join']['Auto Retry Assasin Event'] = v
        Save()
    end ,
})
Tab.Join_SettingInGames:addToggle({
    title = "Auto Retry [Story,Portal,Christmas]",
                ...
                if SaveSettings['Macro']['Play Macro'] and isfile("HolyShz/AnimeAdventures/Macro/" .. tostring(MacroMapSelect)..".json") then
                    ...
                    for i,v in pairs(Macro) do
                        ...
                        if v["Method_Data"]["Method"] == "Place" then
                            ...
                            if GetLevelData._challenge == "double_cost" then
                                repeat task.wait()
                                    MacroStats.Options:ChangeText('Status: Playing Marcro [ ' .. v["Method_Data"]["Method"] .. " ] [" .. v["Index"] .. "/" .. math.max(unpack(TableIndex)) .."]\nTower : " .. v["Method_Data"]["Data"]["UnitPlace"] .. "\n Waiting For : $" .. comma_value(tonumber(Module_Units[MacroData['UnitPlace']]['cost'] * 1.5)) .. " , Time : " ..string.format("%.2f", Timelapse() - StartRecordTime) .. "/" .. string.format("%.2f", tonumber(v["Time"] + tonumber(SaveSettings["Macro"]['Delat Macro Play Time (sec)']))) )
                                until (game:GetService("Players").LocalPlayer._stats.resource.Value >= tonumber(Module_Units[MacroData['UnitPlace']]['cost'] * 1.5) )
                                local args = {
                        ...
                        elseif v["Method_Data"]["Method"] == "Upgrade" then
                            ...
                            for index, value in pairs(workspace._UNITS:GetChildren()) do
                                if value:FindFirstChild("NumberTower") and value.NumberTower.Value == tonumber(v["Method_Data"]["Data"]["UnitPlace"]) then
                                    ...
                                    if GetLevelData._challenge == "double_cost" then
                                        repeat task.wait()
                                            MacroStats.Options:ChangeText('Status: Playing Marcro [ ' .. v["Method_Data"]["Method"] .. " ] [" .. v["Index"] .. "/" .. math.max(unpack(TableIndex)) .."]\nTower : " .. value.Name .. "\n Waiting For : $" .. tonumber(Module_Units[_stats:WaitForChild("id").Value]['upgrade'][tonumber(MacroData['Upgrade'])]["cost"] * 1.5) .. " , Time : " ..string.format("%.2f", Timelapse() - StartRecordTime) .. "/" .. string.format("%.2f", tonumber(v["Time"] + tonumber(SaveSettings["Macro"]['Delat Macro Play Time (sec)']))) )
                                        until (game:GetService("Players").LocalPlayer._stats.resource.Value >= tonumber(Module_Units[_stats:WaitForChild("id").Value]['upgrade'][tonumber(MacroData['Upgrade'])]["cost"] * 1.5) )
                                        local args = { [1] = value }
            ...
            GameFinished:GetPropertyChangedSignal("Value"):Connect(function()
                if GameFinished.Value == true then
                    ...
                    repeat task.wait() 
                        ...
                        local GetLevelData = game.workspace._MAP_CONFIG:WaitForChild("GetLevelData"):InvokeServer()
                        --Replay_Assasin
                        if SaveSettings['Join']['Auto Retry Assasin Event'] then
                            for i,v in pairs(GetAssasinData()) do
                            ...
                            wait(.25)
                        elseif SaveSettings['Join']['Auto Retry In Games'] then
                            if GetLevelData._is_map_or_portal_level == true then
