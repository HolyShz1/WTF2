...
Tab.Join_Challenges:addMulitDropdown({
    ...
    list = {1,2,3,4,5,6},
    default = SaveSettings['Join']['Select Challenges Act'] ,
    callback = function(v)
        SaveSettings['Join']['Select Challenges Act']  = v
        Save()
    end;
})
Tab.Join_Challenges:addMulitDropdown({
        ...
        while task.wait() do
            ...
            if SaveSettings['Join']['Auto Join'] and SaveSettings['Join']['Enable Farm Challenges'] then
                ...
                for i,v in pairs(game:GetService("Workspace"):FindFirstChild("_CHALLENGES"):FindFirstChild("Challenges"):GetChildren()) do
                    ...
                    local ChallengeMapNow = v:FindFirstChild('Level')
                    local ChallengeNow = v:FindFirstChild('Challenge')
                    
                    local MapID = string.split(ChallengeMapNow.Value, "_level_")
                

                    if table.find(SaveSettings['Join']['Select Challenges'],ChallengeCCID[ChallengeNow.Value]) and CheckActive.Value == false then

                        table.foreach(MapID,function(i,v)
                            local MapName = v
                            local MapNum = v
                            if i == 1 then MapName = v end
                            if i == 2 then MapNum = v end

                            if table.find(MapData[SaveSettings['Join']['Select Challenges Map']],MapDataID[MapName]) and tonumber(SaveSettings['Join']['Select Challenges Act']) == MapNum then

                                local args = {  [1] = tostring(Room) }
                                game:GetService("ReplicatedStorage").endpoints.client_to_server.request_join_lobby:InvokeServer(unpack(args))
                                
                                warn(Room)
                                warn(ChallengeNow.Value)
                                warn(ChallengeCCID[ChallengeNow.Value])
                                warn(CheckActive.Value)
